...
def bonus_balls(index, balls, bonus):
    if bonus == 1:
...
def bowling(balls):
    ...
    frame = 0
    index = 0
    first_frame_ball = True
    while(frame < 10):
        score = balls[index]
        if first_frame_ball:
            if score == 10:
                total = total + bonus_balls(index, balls, 2)
                frame = frame + 1
            else:
                first_frame_ball = False
        elif not first_frame_ball:
            if balls[index] + balls[index-1] == 10:
                total = total + bonus_balls(index, balls, 1)
            frame = frame + 1
            first_frame_ball = True
        total = total+ score
        index = index + 1
